...
| 模块 | 文件 | 职责 |
|------|------|------|
| `AppColors` | `Sources/AppLib/Design/AppColors.swift` | 全局功能语义色唯一来源，同时提供 SwiftUI `Color` 与 AppKit `NSColor`：Activity、Information/Time、Attention、Critical、Success 和 Claude/Codex Identity；Buddy/F1 装饰色不纳入状态语义 |
...
| `UsageProgressTrack` | `Sources/AppLib/Usage/UsageProgressTrack.swift` | 共享 5h/7d 配额轨道：用量填充 + 已度过窗口时间；Icon/Overlap 每 30s 更新；Overlap 使用 32% 钢蓝时间层和 65% 的 1px 端线，时间长于用量时在下层，否则在上层（含相等），不用于 session context bar |
