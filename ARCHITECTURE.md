...
| `UsageProgressTrack` | `Sources/AppLib/Usage/UsageProgressTrack.swift` | 共享 5h/7d 配额轨道：用量填充 + 已度过窗口时间；Icon/Overlap 每 30s 更新；Overlap 使用 42% 钢蓝时间层和 65% 的 1px 端线，时间长于用量时在下层，否则在上层（含相等），不用于 session context bar |
